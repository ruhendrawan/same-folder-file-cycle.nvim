...
M.split_mode = "vsplit" -- horizontal "split", or vertical "vsplit"
M.extensions = {
	primary = "ts",
	secondary = "html",
...
local function open_file(new_file, opts)
	if new_file then
...
local function go_other_ext(base_name, extension, opts)
	local new_file = get_related_file(base_name, extension)
	open_file(new_file, opts)
end
	...
	if current_ext == "." .. target_ext then
		if current_ext == "." .. M.extensions.primary then
			...
			if last_related_file and vim.fn.filereadable(last_related_file) == 1 then
				open_file(last_related_file, opts)
			-- or to tempate file (.html)
			else
				go_other_ext(base_name, M.extensions.secondary, opts)
			end
		else
			-- from related file to main code (.ts)
			go_other_ext(base_name, M.extensions.primary, opts)
		end
	else
		go_other_ext(base_name, target_ext, opts)
	end
end
